...
#include "AI/Task/SBTTask_SetFocus.h"
...
EBTNodeResult::Type USBTTask_SetFocus::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	if (AIPawn)
	{
		...
		if (AIController)
		{
			...
			if (EnemyController)
			{				
				EnemyController->SetIsFocusing(true);
